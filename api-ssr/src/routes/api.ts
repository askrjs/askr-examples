...
export function registerApiRoutes(api: AskrAppApi<AppDependencies>, schemas: ApiSchemas): void {
  api
    .get("/health", (ctx) =>
      ctx.ok({
        ok: true,
  ...
  api
    .get("/session", (ctx) => ctx.ok(ctx.auth))
  ...
  api
    ...
    .summary("Create the deterministic demo session")
    .tags("Session")
    .access(requireAnonymous(), security.none())
    .created(schema.object({ authenticated: schema.boolean() }))
    .seeOther()
    .forbidden();

  api
    .delete("/session", async (ctx, { sessions }) => {
      if (ctx.auth.session) await sessions.delete(ctx.auth.session.id);
      return ctx.clearCookie(ctx.noContent(), SESSION_COOKIE, {
        path: "/",
        sameSite: "lax",
        httpOnly: true,
      });
    ...
    .summary("Delete the current demo session")
    .tags("Session")
    .access(requireUser(), security.require("cookieSession"))
    .noContent();

  api
    .get("/dashboard", async (ctx, { dashboard }) => ctx.ok(await dashboard.get(ctx.signal)))
    .operationId("getDashboard")
    .summary("Get workspace dashboard data")
    .tags("Workspace")
    .access(requirePermission("dashboard:read"), security.require("cookieSession"))
    .ok(schemas.Dashboard);

  api
    .get("/activity", async (ctx, { activity }) => ctx.ok(await activity.list(ctx.signal)))
    .operationId("listActivity")
    .summary("List workspace activity")
    .tags("Workspace")
    .access(requireUser(), security.require("cookieSession"))
    .ok(schema.array(schemas.Activity));

  api
    .get("/users", async (ctx, { users }) => ctx.ok(await users.list(ctx.signal)))
    .operationId("listUsers")
    .summary("List users")
    .tags("Users")
    .access(requirePermission("users:read"), security.require("cookieSession"))
    .ok(schema.array(schemas.User));
  ...
  api
    .patch("/users/{id}", {
      ...
      async handler(ctx, input, { users }) {
        ...
        if (!update)
          return ctx.badRequest("name, email, or role must contain a valid editable value");
        const expectedVersion = parseVersion(input.headers["if-match"]);
        if (!Number.isInteger(expectedVersion))
          return ctx.badRequest("If-Match must contain a numeric version");
  ...
  api
    .put("/policies/{id}", {
      ...
      async handler(ctx, input, { policies }) {
        try {
          JSON.parse(input.body.source);
        } catch {
          return ctx.badRequest("source must contain valid JSON");
        }
        const expectedVersion = parseVersion(input.headers["if-match"]);
        if (!Number.isInteger(expectedVersion))
          return ctx.badRequest("If-Match must contain a numeric version");
        ...
        if (result.kind === "conflict")
          return ctx.conflict(`Policy version ${result.current.version} is current`);
