...
function PolicyEditor({ policy }: { policy: Policy }) {
  ...
  const save = createMutation<{ source: string; version: number }, Policy>({
    action: async (input, { signal }) =>
      responseJson(
        await fetch(`/api/policies/${encodeURIComponent(policy.id)}`, {
          method: "PUT",
  ...
  return (
    <>
      <PageHeader
        title={policy.name}
        description="Edit and persist this JSON policy with Monaco."
      />
      <Card>
      ...
      <Button
        ...
        onPress={() => void save.execute({ source: source(), version: policy.version })}
      >
