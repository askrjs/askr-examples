...
function UserEditor({ user }: { user: User }) {
  const update = action<InferSchema<typeof updateUserNameAction.input>, User>(updateUserNameAction);
  const initialError = update.state().error as ActionValidationError | undefined;
  const submittedName =
    initialError?.kind === "invalid" && typeof initialError.values.name === "string"
      ...
      <Dialog open={dialogOpen()} onOpenChange={setDialogOpen}>
        ...
        <DialogPortal>
          ...
          <DialogContent style={{ width: "min(32rem, calc(100vw - 2rem))" }}>
            ...
            <DialogDescription>
              Changes use optimistic version matching and invalidate active user queries.
            </DialogDescription>
            <ActionForm
