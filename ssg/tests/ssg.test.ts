...
afterEach(async () => Promise.all(outputs.splice(0).map((path) => rm(path, { recursive: true, force: true }))));
...
describe('SSG runbook reference', () => {
  it('should expand entries and atomically emit complete documents with metadata', async () => {
    ...
    const metadata = JSON.parse(await readFile(join(outputDir, 'metadata.json'), 'utf8')) as { totalRoutes: number; successful: number };
