...
describe('API-only reference', () => {
  ...
  it('should preserve bind precedence and read If-Match explicitly', async () => {
    ...
    const response = await app.fetch(new Request(
      'http://example.test/api/users/1?id=query&name=Query%20Wins',
      {
        method: 'PATCH',
        headers: {
    ...
    const stale = await app.fetch(new Request('http://example.test/api/users/1', {
      method: 'PATCH',
    ...
    const missingHeader = await freshApp().fetch(new Request('http://example.test/api/users/1', {
      method: 'PATCH',
  ...
  it('should isolate mutable dependencies between app instances', async () => {
    ...
    await first.fetch(new Request('http://example.test/api/users/1', {
      method: 'PATCH',
      headers: { 'content-type': 'application/json', 'if-match': '1' },
