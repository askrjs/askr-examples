...
export function registerUserRoutes(api: ApiDefinition<AppDependencies>, User: Schema): void {
  api
    .get("/api/users", async (ctx, { users }) => ctx.ok(await users.list()))
  ...
  api
    .get("/api/users/{id}", async (ctx, { users }) => {
  ...
  api
    .patch("/api/users/{id}", async (ctx, { users }) => {
    ...
    .tags("Users")
    .pathParam("id", schema.string())
    .headerParam("If-Match", schema.string(), {
      required: true,
      description: "Quoted user version",
    })
    .jsonBody(
      schema.object({
